...
| CANT-8 | **L'harness dei documenti** — *la strada e' scelta: **prima la logica, poi la finestra**. **Tre pezzi fatti**: `nova-harness` — come un documento si divide in blocchi e quale blocco risponde (D269→D272); `nova-harness::modifica` — la proposta, l'anteprima che e' lo stesso codice dell'applicazione, e il rifiuto di scrivere sopra un file cambiato sotto (D273, D274); e `nova-docx` — la **modifica chirurgica**, che e' il premio di D237: si tocca solo `word/document.xml` e ogni altra parte si ricopia byte per byte, provato su un `.docx` vero (D275, D276). Restano: aprire il PDF e il `.docx` in lettura, e il banco delle prove* | ~3.100, di cui **1.518 senza una riga di Qt** | Non e' «2.900 righe di finestra Qt»: solo `harness_finestra.py` tocca Qt, e di quelle 1.582 righe la parte Qt sono 164. La finestra e' un'altra domanda, e da quando il guscio e' Tauri con l'interfaccia in HTML, e' la domanda «una quarta finestra del guscio?» invece di «quali widget?» |
