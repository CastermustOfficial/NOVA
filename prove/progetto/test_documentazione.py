...
# Gli script del repository non stanno piu' tutti in cima: le prove sono in
# `prove/`, gli attrezzi in `attrezzi/`, i banchi di prestazione in `misure/`.
# Un elenco fermo alla radice avrebbe chiamato «sparito» ogni file ordinato.
ALTROVE = ("prove", "attrezzi", "misure")
esistenti = {Path(m).name for m in moduli} | {f.name for f in RADICE.glob("*.py")} | {
    f.name for d in ALTROVE for f in (RADICE / d).rglob("*.py")}
