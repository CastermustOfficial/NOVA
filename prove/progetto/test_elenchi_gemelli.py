...
GEMELLI = [
    ...
    ("nova-harness/src/lib.rs", "NON_GUARDARE",
     ...
     "in Python e' un insieme: l'ordine non c'e'"),
    ("nova-harness/src/prova.rs", "CARTELLE_DI_PROVA",
     lambda: costante("nova/harness_prova.py", "CARTELLE_DI_PROVA"), True, ""),
...
def dove_sta(nome_prova: str):
    """La prova con quel nome, ovunque sia finita sotto `prove/`.

    Si cerca invece di scrivere la cartella: un percorso scritto a mano qui
    dentro e' una cosa che si rompe il giorno che le prove si riordinano —
    ed e' successo.
    """
    trovate = sorted((RADICE / "prove").rglob(nome_prova))
    return trovate[0] if len(trovate) == 1 else None


bugiardi = [f"{n}: {p} non lo nomina" for n, p in ALTROVE.items()
            if dove_sta(p) is None
            or n not in dove_sta(p).read_text(encoding="utf-8", errors="replace")]
